...
const deleteEvent = async (req, res) => {
  try {
    ...
    if (registrationCount > 0) {
      // Allow deletion but with cascade delete - delete all related data first
      console.log(`Deleting event ${eventId} with ${registrationCount} participants`);

      // Delete related data in correct order
      // 1. Delete certificates first
      await sequelize.query(`DELETE FROM certificates_issued WHERE event_id = ?`, {
        replacements: [eventId]
      });

      // 2. Delete daily attendances
      await sequelize.query(`DELETE FROM DailyAttendances WHERE event_id = ?`, {
        replacements: [eventId]
      });
