...
const getUserCertificates = async (req, res) => {
  try {
    ...
    console.log('Getting certificates for user ID:', userId);
      ...
      include: [
        {
          model: Event,
          as: 'event',
          required: false, // LEFT JOIN to avoid issues if event is deleted
...
const downloadCertificate = async (req, res) => {
  try {
    ...
    if (!certificate) {
      console.log('Certificate not found for user:', { certificateId, userId });
      return res.status(404).json({
        success: false,
    ...
    console.log('Certificate found:', certificate.certificate_url);
...
// Regenerate missing certificate files
const regenerateMissingCertificates = async (req, res) => {
  try {
    console.log('=== REGENERATE MISSING CERTIFICATES ===');
    ...
    for (const cert of certificates) {
      try {
        ...
        if (cert.certificate_url) {
          ...
          try {
            await fs.access(filepath);
            console.log(`✅ File exists for certificate ${cert.id}`);
            skipped++;
            continue;
          } catch (fileError) {
            console.log(`❌ Missing file for certificate ${cert.id}: ${cert.certificate_url}`);
          }
        }

        // Regenerate certificate
        console.log(`🔄 Regenerating certificate ${cert.id}...`);

        if (!cert.event || !cert.user) {
          console.log(`❌ Missing event or user data for certificate ${cert.id}`);
          failed++;
          continue;
        }

        // Generate QR Code
        const verificationUrl = `${process.env.APP_URL || 'http://localhost:3000'}/verify-certificate/${cert.certificate_number}`;
        const qrCodeDataUrl = await QRCode.toDataURL(verificationUrl);
        ...
        doc.fontSize(10).font('Helvetica').fillColor('#7f8c8d')
          .text(cert.certificate_number, 0, 150, { align: 'center' });

        doc.fontSize(14).fillColor('#34495e')
          .text('Diberikan kepada', 0, 180, { align: 'center' });

        doc.fontSize(28).font('Helvetica-Bold').fillColor('#2980b9')
          .text(cert.user.nama_lengkap.toUpperCase(), 0, 210, { align: 'center' });

        doc.fontSize(12).font('Helvetica').fillColor('#34495e')
          .text('telah menyelesaikan pelatihan', 0, 260, { align: 'center' });

        doc.fontSize(16).font('Helvetica-Bold').fillColor('#2c3e50')
          .text(cert.event.judul, 0, 285, { align: 'center', width: 700 });

        const eventDate = new Date(cert.event.tanggal).toLocaleDateString('id-ID', {
          day: 'numeric', month: 'long', year: 'numeric'
        });

        doc.fontSize(11).font('Helvetica').fillColor('#7f8c8d')
          .text(`Diselenggarakan oleh ${cert.event.penyelenggara || 'EventHub'}`, 0, 330, { align: 'center' })
          .text(`pada tanggal ${eventDate}`, 0, 350, { align: 'center' });

        if (cert.event.durasi_hari > 1) {
          doc.text(`selama ${cert.event.durasi_hari} hari pelatihan`, 0, 370, { align: 'center' });
        }

        // Issue date
        const issueDate = new Date(cert.issued_date).toLocaleDateString('id-ID', {
          day: 'numeric', month: 'long', year: 'numeric'
        });
        ...
        // Signature
        doc.fontSize(11).font('Helvetica-Bold').fillColor('#2c3e50')
          .text(cert.event.penyelenggara || 'PANITIA PENYELENGGARA', 0, 460, { align: 'center' });
        ...
        const filename = `cert-${cert.event_id}-${cert.user_id}-${Date.now()}.pdf`;
        const filepath = path.join(issuedDir, filename);
        await fs.writeFile(filepath, pdfBuffer);
        ...
        console.log(`✅ Regenerated certificate ${cert.id}: ${filename}`);
        regenerated++;

      } catch (error) {
        console.error(`❌ Failed to regenerate certificate ${cert.id}:`, error.message);
        failed++;
      }
    }
    ...
    console.error('Regenerate certificates error:', error);
    res.status(500).json({
      success: false,
