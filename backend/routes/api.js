...
router.get('/events/:eventId/ticket-categories', async (req, res) => {
  try {
    ...
    // Create default ticket categories based on event price
    const ticketCategories = [];

    if (event.biaya === 0 || event.biaya === '0.00') {
      // Free event - only one free ticket type
      ...
      // Paid event - create regular ticket
      ticketCategories.push({
        id: 1,
        name: 'Tiket Regular',
        description: `Tiket untuk ${event.judul}`,
        price: parseFloat(event.biaya),
        max_quantity: 1,
        is_active: true,
        event_id: parseInt(eventId)
      });
