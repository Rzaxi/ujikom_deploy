...
const Hero = () => {
  ...
  const [eventDestinations, setEventDestinations] = useState([]);
  ...
  // Fetch 5 events from database immediately
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Transform even if less than 5 events
        if (events.length > 0) {
          // Pad with default if less than 5
          while (events.length < 5) {
            events.push({
              id: `default-${events.length}`,
              judul: 'Coming Soon',
              kategori: 'Event',
              flyer_url: null,
              deskripsi: 'More events coming soon!'
            });
          }

          const transformedEvents = events.slice(0, 5).map((event, index) => {
            // Check if flyer_url already has http:// or starts with /
            let imageUrl = "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=300&h=400&fit=crop";
            let backgroundUrl = "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1920&h=1080&fit=crop";
            
            if (event.flyer_url) {
              // If already has http://, use as is
              if (event.flyer_url.startsWith('http://') || event.flyer_url.startsWith('https://')) {
                imageUrl = event.flyer_url;
                backgroundUrl = event.flyer_url;
              } else {
                // If starts with /, prepend base URL
                imageUrl = `http://localhost:3000${event.flyer_url}`;
                backgroundUrl = `http://localhost:3000${event.flyer_url}`;
              }
            }
            
            return {
              id: event.id,
              title: (event.judul || event.title || "EVENT").toUpperCase(),
              region: (event.kategori || event.category || "CATEGORY").toUpperCase(),
              image: imageUrl,
              backgroundImage: backgroundUrl,
              description: event.deskripsi || event.description || "Join this amazing event!",
              shortDescription: event.judul || event.title || "Event"
            };
          });

          setEventDestinations(transformedEvents);
          setIsDataReady(true);
        }
      } catch (error) {
        ...
        setEventDestinations([
          {
            ...
            description: "Join amazing events and workshops",
            shortDescription: "Featured Event"
          },
          {
            id: 2,
  ...
  const getCurrentDestination = () => {
    if (eventDestinations.length === 0) return null;
  ...
  useEffect(() => {
    // Only start auto-rotate after data is ready
  ...
  return (
    <section className={`relative min-h-screen w-full overflow-hidden transition-opacity duration-500 ${!isDataReady ? 'opacity-0' : 'opacity-100'}`} style={{ boxShadow: 'none !important' }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{
            backgroundImage: `url('${getCurrentDestination()?.backgroundImage || eventDestinations[0]?.backgroundImage || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1920&h=1080&fit=crop'}')`,
            willChange: 'opacity'
          }}
        />
      </AnimatePresence>
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-12 items-center min-h-screen">
            ...
            <div className="lg:col-span-6 relative px-4 sm:px-6 lg:px-8 pt-20 lg:pt-0 text-center lg:text-left">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {/* Small region label - moved to top */}
                  <div className="mb-3 sm:mb-4">
                    <span className="text-white/80 text-xs sm:text-sm lg:text-base font-medium tracking-wider uppercase">
                  ...
                  <div className="mb-4 sm:mb-6 lg:mb-8">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white leading-tight tracking-tight break-words">
                      {getCurrentDestination()?.title || "LOADING..."}
                    </h1>
                  </div>

                  {/* Dynamic description with fixed height */}
                  <div className="mb-6 sm:mb-8 h-20 sm:h-24 lg:h-28">
                    <p className="text-white/90 text-sm sm:text-base lg:text-lg leading-relaxed max-w-sm sm:max-w-md lg:max-w-lg mx-auto lg:mx-0 line-clamp-3">
              ...
              {/* Rounded button with play icon - links to specific event */}
              <Link
                to={`/events/${getCurrentDestination()?.id || ''}`}
