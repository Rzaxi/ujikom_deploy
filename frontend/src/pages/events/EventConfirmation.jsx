...
const EventConfirmation = () => {
  ...
  const handleConfirmRegistration = async () => {
    ...
    try {
      ...
      const eventPrice = event?.biaya || 0;

      if (eventPrice > 0) {
        // Paid event - create payment with Xendit
        const response = await eventsAPI.createPayment(id);

        if (response.data.success) {
          const { invoice_url, order_id } = response.data.data;
        ...
        const response = await eventsAPI.registerFree(id);
        toast.success('Berhasil mendaftar event gratis!');
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="p-6 space-y-4">
                {ticketTypes.map((ticket) => (
                  <div
                    key={ticket.id}
                    className={`relative border rounded-lg p-6 cursor-pointer transition-all ${selectedTicketType === ticket.id
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {ticket.badge && (
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${ticket.badgeColor === 'green' ? 'bg-green-100 text-green-800' :
                          ...
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${ticket.available
                            ? 'bg-green-100 text-green-800'
              ...
              <div className="p-6">
                <div className="flex items-start space-x-3">
                  <button
                    ...
                    className={`mt-1 w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${agreedToTerms
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 sticky top-24">
              ...
              <div className="p-6">
                ...
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-lg font-medium text-sm transition-colors ${agreedToTerms && !isRegistering && selectedTicket?.available
