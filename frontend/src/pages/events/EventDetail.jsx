...
const EventDetail = () => {
  ...
  // Define fetchRelatedEvents first (before useEffect)
  const fetchRelatedEvents = useCallback(async (kategori) => {
    try {
      ...
      let filtered = allEvents.filter(event => {
        return event.kategori === kategori && event.id !== parseInt(id);
      });
  ...
  // Fetch related events only once when event category is known
  useEffect(() => {
    if (eventCategory) {
      fetchRelatedEvents(eventCategory);
    }
              ...
              <div className="absolute top-4 right-4 z-20">
                <div className="flex items-center space-x-3">
                  <button
                    ...
                    className={`p-3 backdrop-blur-sm rounded-full transition-all duration-300 border ${isBookmarked
